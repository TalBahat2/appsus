...
function removeReview(bookId, reviewId) {
  console.log('book in service', bookId);
  console.log('reviewId in service', reviewId);
...
function saveRev(review, bookId) {
...
function searchAxios(searchWords) {
  console.log(searchWords);

  var searchBooks = utilService.loadFromStorage(BOOKS_DB) || {}
  if (searchBooks && searchBooks[searchWords]) return Promise.resolve(searchBooks[searchWords])
  return axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${searchWords}`)
    .then(res => {
      console.log('getting data from server');
      searchBooks[searchWords] = res.data.items
      utilService.saveToStorage(BOOKS_DB, searchBooks)
